...
void main_boing_ball_fx(u16 vbl_to_exit){
	...
	while (vblCount < vbl_to_exit){
		...
		vblCount01 += hscrollInc;
		if (vblCount01 >= PERSPECTIVE_STEP)
		{
			vblCount01 = PERSPECTIVE_STEP - 1;
			hscrollInc = -1;
		}
		else
		if (vblCount01 < 0)
		...
		VDP_setHorizontalScrollLine(PLAN_B, 2, scroll_PLAN_B[vblCount01], TABLE_LEN, TRUE);
	}
}
